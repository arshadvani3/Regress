"""Best-effort redaction of likely-sensitive content from trace text.

Per CLAUDE.md's Quality bar: "Sanitize any real trace content before it lands
in generated evals or docs (strip emails, keys, names)." It runs in two
places: EvalGen (before a real trace becomes a committed eval) and, when
`REGRESS_SANITIZE_INGEST` is enabled, at ingest (before raw prompt/response
text is persisted at all). Both share this one implementation.
...
def sanitize_message_content(content: dict[str, object]) -> dict[str, object]:
    """Sanitize a single `Message.content` dict (one `{role, parts: [...]}`),
    the row shape the store persists. A thin single-message wrapper over
    `sanitize_messages` so ingest can redact each message as it's built."""
    return sanitize_messages([content])[0]
